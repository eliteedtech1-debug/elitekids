...
export default function TeacherQuickCreate() {
  ...
  const createQuiz = async () => {
    ...
    if (!form.class_code.trim()) return toast.error('Class code required');
    setCreating(true);
    try {
  ...
  const saveQuestions = async () => {
    ...
    if (valid.length === 0) return toast.error('Add at least 1 complete question');
    setSaving(true);
    try {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6">
      <h1 className="mb-1 flex items-center gap-2 text-xl font-extrabold text-gray-800">
        <Edit3 className="h-6 w-6 text-blue-500" /> Quick Create Quiz
      </h1>
      <p className="mb-5 text-sm text-gray-500">Create a quiz in 2 minutes from your phone. Add questions, publish, done!</p>

      {/* Create Form */}
      <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <h3 className="mb-3 text-sm font-extrabold text-gray-700">New Quiz</h3>
        <div className="grid grid-cols-2 gap-3">
          <label className="col-span-2 text-xs font-bold text-gray-600">
            Title *
            <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="My Fun Quiz" className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" />
          </label>
          <label className="text-xs font-bold text-gray-600">
            Subject
            <input value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} placeholder="Math" className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" />
          </label>
          <label className="text-xs font-bold text-gray-600">
            Class Code *
            <input value={form.class_code} onChange={e => setForm({ ...form, class_code: e.target.value })} placeholder="CLS0610" className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" />
          </label>
        </div>
        <button onClick={createQuiz} disabled={creating} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-blue-500 px-5 py-2.5 text-sm font-extrabold text-white shadow hover:bg-blue-600 disabled:opacity-50">
          {creating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Create Quiz
        </button>
      </div>
      ...
      {editingQuiz && (
        <div className="mb-6 rounded-2xl border-2 border-blue-200 bg-blue-50/50 p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-extrabold text-blue-700">📝 Add Questions</h3>
            <button onClick={() => setEditingQuiz(null)} className="text-xs font-semibold text-gray-400 hover:text-gray-600">Close</button>
          </div>
          ...
          {questions.map((q, idx) => (
            <div key={idx} className="mb-3 rounded-xl bg-white p-3 shadow-sm">
              <button onClick={() => setExpandedQ(expandedQ === idx ? -1 : idx)} className="flex w-full items-center justify-between text-left">
                <span className="text-xs font-bold text-gray-600">Q{idx + 1}. {q.question_text ? q.question_text.slice(0, 40) + (q.question_text.length > 40 ? '...' : '') : '(empty)'}</span>
                {expandedQ === idx ? <ChevronUp className="h-4 w-4 text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-400" />}
              </button>

              {expandedQ === idx && (
                <div className="mt-3 space-y-2">
                  <textarea value={q.question_text} onChange={e => updateQuestion(idx, 'question_text', e.target.value)} placeholder="Type your question..." className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" rows={2} />
                  ...
                  {(['A', 'B', 'C', 'D'] as const).map(opt => (
                    <div key={opt} className="flex items-center gap-2">
                      ...
                      <input value={q[`option_${opt.toLowerCase()}` as keyof Question] as string} onChange={e => updateQuestion(idx, `option_${opt.toLowerCase()}` as keyof Question, e.target.value)} placeholder={`Option ${opt}`} className="flex-1 rounded-lg border border-gray-200 px-3 py-1.5 text-sm" />
                    </div>
                  ))}
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button onClick={saveQuestions} disabled={saving} className="inline-flex items-center gap-1 rounded-lg bg-blue-500 px-4 py-2 text-xs font-bold text-white shadow hover:bg-blue-600 disabled:opacity-50">
              {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle className="h-3.5 w-3.5" />} Save Questions
            </button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-extrabold uppercase tracking-wide text-gray-400">Your Quizzes</h2>
        <button onClick={load} className="text-xs font-semibold text-blue-500 hover:underline">Refresh</button>
      </div>

      {loading ? (
        <div className="mt-8 flex justify-center"><Loader2 className="h-6 w-6 animate-spin text-gray-300" /></div>
      ) : quizzes.length === 0 ? (
        <p className="mt-8 text-center text-sm text-gray-400">No quizzes yet — create one above!</p>
      ) : (
          ...
          {quizzes.map(q => (
            <div key={q.id} className={`rounded-2xl border bg-white p-4 shadow-sm ${q.status === 'published' ? 'border-green-200' : 'border-gray-100'}`}>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-3 text-xs text-gray-500">
                <span>📝 {q.actual_questions || q.question_count} questions</span>

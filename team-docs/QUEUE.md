...
| Q25 | Q2 speech FE leaf (NO cross-lane deps): Pronunciation Coach + ReadingTracker components over LIVE /kids/speech/assess + /kids/speech/progress endpoints (i18n EN+HA, vitest, no backend changes) — briefs/q2-opencode-lowdep.md | opencode → Buffy (takeover, opencode unavailable) | IMPLEMENTED 2026-09-03 (Buffy takeover — tsc clean, vitest 128/128, build OK; +i18n chunk refactor en.ts → chunks/en-a-c…w-z sorted; reports/q2-speech-fe-progress.md; uncommitted) |
| Q26 | Q2 drawing FE components (Q2-G, NO backend/ML deps): DrawingCanvas + TracingGuide + DrawingFeedback (pure canvas; TensorFlow.js recognition plugs in later, OUT of scope) — briefs/q2-opencode-lowdep.md | opencode → Buffy (takeover) | IMPLEMENTED 2026-09-03 (Buffy takeover — tsc clean, vitest 145/145, build OK; DrawingCanvas DPR-aware + TracingGuide ghost/live-% + DrawingFeedback stars 1–3; /student/drawing?mode=demo; reports/q2-speech-fe-progress.md; uncommitted) |
| Q27 | Q2 speech templates story/count (completes Q2-B 5/5) + registry/enum/i18n — QUEUE only AFTER Q24 lands (engine bridge is the play path) | opencode → Buffy (takeover) | IMPLEMENTED 2026-09-03 (Buffy takeover — speech-story + speech-count registered: VALID_TEMPLATES (kids.js), fix-template-enum.js 15 values, GameCreator cards + config templates, GamePlay mode map, i18n EN+HA; Q2-B now 5/5; full backend suite back to 499/2 baseline; reports/q2-speech-fe-progress.md; uncommitted) |
| Q28 | Q2-E Portfolio backend v1 (roadmap §2.7): skill map (ADE v2) + evidence (speech logs + game sessions) + recommendations + JSON export — read-only aggregation, pure helpers, q2-portfolio.test.js 7/7 | Buffy (worker) | MERGED 2026-09-03 (commit daff474, pushed origin/main — auto-deploy ran; q2-portfolio.test.js 7/7 pass; gate: garden-companion 2 pre-existing fails, b1-regression flaky; reports/q2-portfolio-progress.md) |
| Q29 | Q2-E Portfolio frontend (SkillMap + EvidenceGallery + export button on ParentDashboard/Teacher view) | opencode → Buffy (takeover) | IMPLEMENTED 2026-09-03 (Buffy takeover — SkillMap mastery bands + EvidenceGallery speaking/games + 7d + JSON export on ParentDashboard child view; i18n 23 keys EN+HA; tsc + vitest 145/145 + build OK; reports/q2-speech-fe-progress.md; uncommitted) |
